...
def main():
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        for x_t, y_t in fun_sm:

            target_x = initial_x + x_t
            target_y = initial_y + y_t

            # 发出飞行指令
            commander.go_to(target_x, target_y, target_altitude,yaw=0.0,duration_s=travel_duration,relative=False)

            # 等待该段飞行完成
            time.sleep(travel_duration + 0.005)  # 加入一点缓冲时间
